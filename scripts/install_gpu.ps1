...
python -c "import noisereduce, nara_wpe, sherpa_onnx; print('Enhancement + Gipformer Python dependencies: OK')"
...
$PythonSherpaLib = python -c "from pathlib import Path; import sherpa_onnx; print(Path(sherpa_onnx.__file__).resolve().parent / 'lib')"
...
python -c "from pathlib import Path; import sherpa_onnx; p=Path(sherpa_onnx.__file__).resolve().parent/'lib'/'onnxruntime_providers_cuda.dll'; assert p.exists(), f'Missing {p}'; print(f'sherpa-onnx CUDA provider: {p}')"
