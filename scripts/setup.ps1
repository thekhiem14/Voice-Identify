...
& $Py -c "import torch; x=torch.ones(2); print(f'PyTorch CPU OK: torch={torch.__version__}, sum={x.sum().item()}')"
& $Py -c "import noisereduce, nara_wpe, sherpa_onnx; print('CPU enhancement and Gipformer dependencies: OK')"
...
& $Py -m pip install -e $DiariZen
& $Py -m pip install -e (Join-Path $DiariZen "pyannote-audio")
